...
export default function Home() {
  ...
  return (
    <main className="container max-w-2xl mx-auto  min-h-screen py-12">
      <div className="flex justify-between items-center">
        <ProgressBar step={step === Steps.UPLOAD_IMAGE ? 1 : step === Steps.CUT_IMAGE ? 2 : 3} totalSteps={3} />
      </div>

      <div className="w-full space-y-8">
        {step === Steps.UPLOAD_IMAGE && (
          <UploadImage onNext={handleNextStep} />
        )}

        {step === Steps.CUT_IMAGE && (
          <Cutter onNext={handleNextStep} />
        )}

        {step === Steps.PREVIEW && (
          <Preview onNext={handleNextStep} />
        )}
      </div>
    </main>
  );
