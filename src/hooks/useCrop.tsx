...
export function useCrop(imageSource: string | HTMLImageElement | null): UseCropResult {
  ...
  const crop = useCallback(
    async (area: CropArea, options: CropOptions = {}): Promise<string | null> => {
      if (!image) {
      ...
      try {
        ...
        if (area.x < 0 || area.y < 0 || area.x + area.width > image.width || area.y + area.height > image.height) {
...
export function useCropWithLines(imageSource: string | HTMLImageElement | null) {
  ...
  const cropByLines = useCallback(
    async (
      ...
      const hLines = [
        0,
        ...horizontalLines.map((pos) => Math.floor(pos * imageHeight)).sort((a, b) => a - b),
        imageHeight
      ];
      const vLines = [0, ...verticalLines.map((pos) => Math.floor(pos * imageWidth)).sort((a, b) => a - b), imageWidth];
